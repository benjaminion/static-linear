...
- **Projects** — project date ranges and milestones only (no task markers or
...
- **Tasks** — blocking graph with date-ordered X placement, status-coloured
  nodes, smooth obstacle-aware routes, hover highlights, and title/due-date chips.
  The three task-mode layouts are computed during the static build, so changing
